...
def k8s_setup_dns(args):
    ...
    run_shell(args,
              'sudo echo "Environment="KUBELET_CGROUP_ARGS=--cgroup-driver='
              'systemd" >> /tmp/10-kubeadm.conf')
    run_shell(args,
              'sudo echo "Environment="KUBELET_EXTRA_ARGS=--fail-swap-on='
              'false" >> /tmp/10-kubeadm.conf')
